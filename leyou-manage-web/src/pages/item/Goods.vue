    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="OnAndDown('0',props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="OnAndDown('1',props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      deleteGoods(id){
        this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
          ...
          this.$http.get("/item/spu/deleteGoods/" +id)
            .then((res) => {
      ...
      OnAndDown(s,id){
        ...
        this.$message.confirm('是否继续'+name, {
          confirmButtonText: '确定'+name,
          cancelButtonText: '取消'+name,
          type: 'warning'
        }).then(() => {

          this.$http.get("/item/spu/OnAndDown",{
            params:{
              saleable:s,
              spuId:id
            }
